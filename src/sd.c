...
{
    Timer2 = 50;    // 500ms
    SD_SSP_ReceiveData(); /* Read a byte (Force enable DO output) */
    do {
        if (SD_SSP_ReceiveData() == 0xFF) return SD_TRUE;
    } while (Timer2);
...
uint8_t SD_SendCommand (uint8_t cmd, uint32_t arg, uint8_t *buf, uint32_t len)
{
    ...
    SD_SSP_SendData(cmd | 0x40);
    SD_SSP_SendData(arg >> 24);
    SD_SSP_SendData(arg >> 16);
    SD_SSP_SendData(arg >> 8);
    SD_SSP_SendData(arg);
    SD_SSP_SendData(crc_stop); /* Valid or dummy CRC plus stop bit */
...
SD_BOOL SD_ReadConfiguration ()
{
    ...
    switch (CardType)
    {
        ...
        case CARDTYPE_SDV2_HC:
            ...
            for (i=64-16;i;i--) SD_SSP_ReceiveData();  /* Purge trailing data */
...
SD_BOOL SD_SendDataBlock (const uint8_t *buf, uint8_t tkn, uint32_t len)
{
    ...
    if (( (SD_SSP_ReceiveData()) & 0x0F) != 0x05)
        return (SD_FALSE); /* write error */

    /* Wait for write complete. */
    Timer1 = 20;  // 200ms
    do {
        recv = SD_SSP_ReceiveData();
        if (recv == 0xFF) break;
    } while (Timer1);
...
void SingleSector_RW_Test()
{
    ...
    printf("Fill sector #%d with incrementing values.\n\r", S_SECTOR_INDEX);
...
void MultiSector_RW_Test ()
{
    ...
    printf("Fill %d sectors from #%d with incrementing values.\n\r", M_SECTOR_NUM, M_SECTOR_INDEX);
    for (i=0;i<512*M_SECTOR_NUM;i++) buf[i] = i*2;
...
void SD_Test(){
	...
	printf("Sector size: %d bytes\n\r", CardConfig.sectorsize);
	printf("Sector count: %d\n\r", CardConfig.sectorcnt);
	printf("Block size: %d sectors\n\r", CardConfig.blocksize);
	printf("Card capacity: %d MByte\n\r", (((CardConfig.sectorcnt >> 10) * CardConfig.sectorsize)) >> 10);
	printf("OCR(hex): ");
